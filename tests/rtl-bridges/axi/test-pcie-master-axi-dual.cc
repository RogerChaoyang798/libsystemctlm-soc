...
SC_MODULE(TopDual)
{
	...
	tlm2axilite_bridge<64, 32 > bridge_tlm2axilite0;
	...
	SC_HAS_PROCESS(TopDual);

	void gen_rst_n(void) { rst_n.write(!rst.read()); }

	TopDual(sc_module_name name, unsigned int ram_size = 8 * 1024)
	...
	, tlm_hw_bridge1("tlm_hw_bridge1", /*base_addr*/0x00000, /*base_offset*/0x0)
